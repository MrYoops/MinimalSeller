...
function StockPageV3() {
  ...
  const syncInventory = async () => {
    try {
      // Временно отключаем автоматическую синхронизацию
  ...
  const handleSyncCatalog = async () => {
    ...
    const confirmed = window.confirm(
      ...
      'Эта операция загрузит актуальные остатки с маркетплейсов в вашу базу данных.\n\n' +
      'Продолжить?'
    )
    ...
    setLoading(true)
    try {
      console.log('🔄 [SYNC-CATALOG] Начинаем импорт остатков...')
      
      // Временно показываем успех
      toast.success('✅ Функция в разработке')
      
    } catch (error) {
      console.error('❌ [SYNC-CATALOG] Ошибка:', error)
      toast.error('Ошибка импорта остатков')
    }
    setLoading(false)
  }
