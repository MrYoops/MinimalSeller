...
function TransactionsPage() {
  ...
  const loadTransactions = async () => {
    ...
    try {
      ...
      alert('Ошибка загрузки транзакций: ' + (error.response?.data?.detail || error.message))
    }
    setLoading(false)
  }
    ...
    <div className=\"space-y-6 pb-8\">
      ...
      {loading ? (
        <div className=\"text-center py-12\">
          <p className=\"text-mm-cyan animate-pulse\">// ЗАГРУЗКА ТРАНЗАКЦИЙ...</p>
        </div>
      ) : transactions.length === 0 ? (
        <div className=\"card-neon text-center py-12\">
          <p className=\"text-mm-text-secondary mb-2\">Нет транзакций за выбранный период</p>
                    ...
                    <React.Fragment key={txn._id}>
                      <tr className=\"border-b border-mm-border hover:bg-mm-gray transition-colors\">
                        ...
                        <td className=\"py-4 px-4 font-mono text-sm text-mm-cyan\">
                          {txn.order_id || txn.posting_number || '-'}
                        </td>
