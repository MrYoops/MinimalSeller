...
from schemas.inventory import Inventory, FBOInventory, InventoryHistory, FBOShipment, FBOShipmentItem, InventoryAdjustment, InventoryResponse, FBOInventoryResponse, InventoryHistoryResponse, FBOShipmentResponse
    ...
    for inv in inventory_list:
        ...
        if product:
            ...
            if photo:
                ...
                # Fallback на старые поля с безопасным доступом
                photos = product.get("photos", [])
                images = product.get("minimalmod", {}).get("images", [])
                product_image = ""
                
                if photos and len(photos) > 0 and photos[0]:
                    product_image = photos[0]
                elif images and len(images) > 0:
                    if isinstance(images[0], str):
                        product_image = images[0]
                    elif isinstance(images[0], dict):
                        product_image = images[0].get("url") or images[0].get("file_name") or ""
...
async def sync_catalog(current_user: dict = Depends(get_current_user)):
    """
    Синхронизация каталога товаров с остатками
    """
    try:
        # Используем существующий функционал синхронизации
        from routers.stock_sync import manual_sync
        
        # Получаем основной склад
        from core.database import get_database
        db = await get_database()
        warehouse = await db.warehouses.find_one({"type": "main"})
        
        if not warehouse:
            raise HTTPException(status_code=404, detail="Main warehouse not found")
        
        # Вызываем синхронизацию всех товаров
        result = await manual_sync(
            data={"warehouse_id": str(warehouse["_id"])},
