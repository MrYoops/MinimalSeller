...
async def list_internal_categories(
    ...
    try:
        ...
        # Get total count
        total = await db.internal_categories.count_documents(query)
        
        # Get categories
        categories = await db.internal_categories.find(query).skip(skip).limit(limit).to_list(limit)
...
async def get_internal_category(
    ...
    try:
        category = await db.internal_categories.find_one({"_id": ObjectId(category_id)})
...
async def create_internal_category(
    ...
    try:
        ...
        existing = await db.internal_categories.find_one({
            "$or": [
        ...
        # Insert
        result = await db.internal_categories.insert_one(category_doc)
...
async def update_internal_category(
    ...
    try:
        # Check if exists
        existing = await db.internal_categories.find_one({"_id": ObjectId(category_id)})
        if not existing:
            raise HTTPException(status_code=404, detail="Category not found")
        ...
        # Update
        await db.internal_categories.update_one(
...
async def delete_internal_category(
    ...
    try:
        ...
        products_count = await db.product_catalog.count_documents({"internal_category_id": category_id})
        if products_count > 0:
            raise HTTPException(
                status_code=400, 
        ...
        # Delete
        await db.internal_categories.delete_one({"_id": ObjectId(category_id)})
...
async def get_merged_attributes(
    ...
    try:
        ...
        if internal_category_id and 'site' in selected_channels:
            category = await db.internal_categories.find_one({"_id": ObjectId(internal_category_id)})
            if category:
