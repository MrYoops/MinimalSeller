...
class KeyService:
    ...
    async def add_key(user_id: str, key_data: APIKeyCreate) -> Dict[str, Any]:
        ...
        # TEMPORARY: Store key without encryption for testing
        encrypted_key = key_data.api_key
        print(f"🔧 TEMPORARY: Storing key without encryption: {encrypted_key[:20]}...")
    ...
    @staticmethod
    async def get_encrypted_key(user_id: str, key_id: str) -> Optional[str]:
